...
# /////////////////////////////////////////////////////////////////////////////
# Sets the logging level of each node in the primaries.txt file to -1
...
function set-log () 
{
    ...
    for p in $(cat primaries.txt); do
        echo "Logging everything for ${p}"
        mongo --quiet  "mongodb://${p}/admin" --ssl --authenticationDatabase admin --username ${USER} --password ${PASS} --eval "db.setProfilingLevel(0, -1);"
    done
